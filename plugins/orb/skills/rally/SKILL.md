...
Rally resolves each with a declared artefact path, a definitive post-design disjointness check, and `<rally-folder>/rally.yaml` as durable state (where `<rally-folder>` is the rally's spec-shaped directory — see §3).
...
Before anything else, scan for an in-flight rally.

```
scan orbit/specs/*/rally.yaml:
  for each rally.yaml found, read and validate (§11 Resumption + §12 Validation)
  if any has phase != "complete":
    that is the active rally — resume from its recorded phase.
    do not start a new rally.
  if zero found, or all found are phase == "complete":
    no active rally — proceed to §2 (Propose the Rally).
```

**One active rally at a time** is non-negotiable. A new rally launched while another is in-flight would scatter sub-agent briefs across overlapping spec directories and make resumption ambiguous. If the scan finds **two or more** rallies with `phase != complete`, treat that as a state error per §12 validation — refuse and name the colliding folders.

**Completed rallies stay in place.** Each rally's folder is its own record — there is no archival step, no tidying. The next rally gets its own folder alongside the completed ones.
...
**Rally folder naming.** Derive a rally slug from the goal string — lowercase, hyphenated, trimmed to ~3–5 evocative words (e.g. goal `"orbit UX uplift via Claude Code tools — mission resilience, live visibility, structured prompts"` → slug `orbit-ux-uplift`). The rally's spec-shaped folder is:

```
orbit/specs/<today-date>-<slug>-rally/
```

The `-rally` suffix keeps it visually distinct from card-level spec folders when scanning `orbit/specs/`. This path is referred to elsewhere in the skill as `<rally-folder>`.

On approval, create `<rally-folder>/` on main and write `rally.yaml` inside it:
...
3. **Snapshot diff (independent verification):** capture a post-snapshot (`git status --porcelain`) and compute the set difference `post \ pre`. Any entry in that difference that is neither under `<spec_dir>` nor on the fixed lead-owned allowlist (`<rally-folder>/rally.yaml`) rejects the sub-agent's output. Entries present in both pre and post are pre-existing lead-side state and are ignored. (constraint #9, ac-04)
...
3. **No archival step.** The rally folder stays where it is — `<rally-folder>/rally.yaml` is the durable record. When the next rally begins, it creates its own folder alongside this one (§1, §3).

### 11. Resumption

When `/orb:rally` is invoked and the §1 scan finds a rally.yaml with `phase != complete` inside some `<rally-folder>/`:

1. Read and validate that rally.yaml on main (§12)
...
**Completion handling:** If the §1 scan finds **only** completed rallies (each `phase == complete`), there is nothing to resume. Proceed straight to §2 (Propose the Rally) — the new rally will create its own folder and live alongside the completed ones.
...
```
rally.yaml at <rally-folder>/rally.yaml is invalid: <specific problem, e.g. "card 2 has status 'implementing' but worktree is null">
...
|-------|------|-------|----------|
| Rally coordination | `orbit/specs/<date>-<slug>-rally/rally.yaml` | Per-card coordination status, implementation order, phase, goal, timestamps | **main only** |
