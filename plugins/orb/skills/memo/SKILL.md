---
name: memo
description: Quickly jot a rough idea and file it in `cards/memos/`
---
...
Check that `cards/memos/` exists. If not, create it (including `cards/` if needed).
...
```
cards/memos/YYYY-MM-DD-<slug>.md
```
...
```
Memo saved: cards/memos/YYYY-MM-DD-<slug>.md
```

Suggest next steps:
- **Write more memos** if there are other ideas floating around
- **`/orb:distill cards/memos/YYYY-MM-DD-<slug>.md`** when ready to turn this into a feature card
