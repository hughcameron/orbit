...
- Use `orbit audit conformance --json` on-demand to read **workflow conformance** — a structured findings envelope covering plugin-canonical-file drift, card-state, memo staleness, and the plugin-version pin. Each finding carries `remediation.verb` the agent runs without translation. Per spec 2026-05-19-workflow-conformance.
