...
References: McGill sprint alignment (disposition in agent definitions), ralph-loop (self-referential iteration), cards/0003 (progress.md pattern).
...
**A:** 3 iterations (hardcoded default). First attempt + two re-designs. Three NO-GOs = strong signal the card needs human rethinking. Matches the observed pattern from McGill where 2-3 spec cycles typically either solve the problem or produce enough evidence to reframe.
...
- **drive.yaml co-located with first spec:** Keeps orchestration state near the work, discoverable by session-context.sh without a new file convention at specs/ root.
