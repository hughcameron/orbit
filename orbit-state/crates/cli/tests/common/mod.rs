...
/// Expected canonical envelope for `card tree 0001-alpha --depth 1` against
...
/// Expected error envelope for `card tree 9999` (unknown id) against the
/// two-related-cards fixture's cards directory.
...
/// Expected error envelope for `card specs 9999` (unknown id).
...
/// Expected error envelope for `graph --card 9999` (unknown id).
...
/// Populate `<root>/.orbit/` with one card (`0001-alpha`) that lists a spec
/// (`s1`), and the spec back-references the card. Used by card.specs parity
/// tests for the healthy bidirectional case.
...
/// field (`legacy_field`). Used by audit.drift parity tests to exercise the
/// permissive-scan path that strict-parse can't reach.
...
/// Alpha is the only orphan (no incoming relations + specs: []).
...
/// Expected canonical envelope for `card specs 0001-alpha` against the
/// linked-spec fixture.
...
/// with the given distillate text. `started_at` and `ended_at` are stamped
/// by the substrate so callers must read them back from disk before
/// comparing — see the parity tests for the read-back pattern.
