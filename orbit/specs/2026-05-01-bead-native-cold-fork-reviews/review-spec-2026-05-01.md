...
**Pass:** 1
**Description:** ac-09 requires creating `orbit/decisions/0012-bead-acceptance-field-as-cold-fork-substrate.md`. But `orbit/decisions/` already has two files numbered 0011: `0011-beads-execution-layer.md` and `0011-design-intent-not-means.md`. The numbering convention is already broken; ac-09 hard-codes 0012 without a check or recovery rule.
**Evidence:** `ls orbit/decisions/` returns `0011-beads-execution-layer.md` and `0011-design-intent-not-means.md` — two distinct decisions sharing the 0011 prefix. There is no AC to resolve the existing 0011 collision before adding 0012.
...
**Category:** missing-requirement
**Pass:** 2
**Description:** ac-06 removes `metadata.test_prefix` lookup from review-pr §3. Decision 0002 (`orbit/decisions/0002-ac-test-prefix.md`) establishes the test_prefix convention. Removing the runtime use without superseding the decision leaves the decision register out of sync with the skill's behaviour.
**Evidence:** `ls orbit/decisions/0002*` exists; ac-06 removes the field reference; no AC updates decision 0002's status.
