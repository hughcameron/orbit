...
3. A downstream repo ran `/orb:setup` once, answered "no" to the prompt, and later ran `mkdir orbit/decisions` by hand.
...
- **The card that produced this spec** (`cards/0008-*.yaml`) — references `orbit/cards/memos/`, `orbit/specs/`, `orbit/decisions/` already (prose in the then: clause, line 28). Already correct.
